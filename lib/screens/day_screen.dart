...
class _DayScreenState extends State<DayScreen>
  ...
  EventSortOption _sortOption =
      EventSortOption.timeAscending;
  ...
  void didUpdateWidget(
    ...
    // Ya no necesitamos invalidar ningún cache.
    //
    // widget.events siempre representa el estado actual
    // proveniente de EventProvider.
  }

  Future<void> _loadSortPreference() async {
    final sortOption =
        await EventSortingUtils.loadSortPreference();

    if (!mounted) return;

    if (sortOption != _sortOption) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    // IMPORTANTE:
    //
    // Estos datos se calculan DIRECTAMENTE desde
    // widget.events en cada build.
    //
    // No existe cache de eventos.
    final sortedEvents =
        EventSortingUtils.sortEvents(
      widget.events,
      widget.day,
      sortBy: _sortOption,
    );

    final eventsByPeriod =
        EventSortingUtils.groupByTimePeriod(
      sortedEvents,
      widget.day,
      sortGroups: false,
    );

    final nonEmptyPeriods =
        eventsByPeriod.entries
            .where(
              (entry) =>
...
class _PeriodSection
  ...
  final bool Function(Event)
      isEventPast;

  final Function(int, Event)
      onUpdateEvent;
